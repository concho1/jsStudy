...
* 자바스크립트에서의 자료형
  ...
  - 논리형(boolean) : 참(true) 또는 거짓(false) 두 가지
                                          값을 가지는 자료형.
  ...
  - undefined : 일반적으로 변수를 만든 후 초기화하지
                                 않은 상태를 말함.
                                 예) let data;
                   alert("data >>> " + data);
  ...
  - null : 아무 것도 참조하고 있지 않다는 의미.
                       ...
                       주로 객체를 담을 변수를 초기화할 때 많이
                       사용이 됨.

  - NaN : Not A Number.
                     즉, 숫자가 아닌 데이터를 숫자처럼 사용할 때
                     나타나는 자료형.
...
var str = 150;
console.log("num >>> " + str);  // js의 헛점 중복이름을 허용함

/*
 * 변수 재선언 및 재할당
 * - var를 사용한 변수는 재선언과 재할당을 할 수 있음.
 * - 간단한 소스라면 변수를 모두 기억할 수 있지만, 수천 줄이나 수만 줄의 소스를 작성하다 보면 실수로 같은 이름을 가진 변수를 다시 선언할 수 있음.
 *   이러한 작업을 여러 사람과 공동으로 작업을 한다면 더욱 더 위험해 질 수 있음.
 *
 */
...
console.log(sum);
/*
 * 놀랍게도 실행 가능하지만 NaN(Not A Number) 뜸
 * 요즘에는 변수 중복선언 방지용으로 let 이라는 타입을 많이 쓴다고 함
 */

const su = 148;
console.log(su);

//su = 227; //상수로 선언했기 때문에 값을 수정하지 못함

//let h = "hello1"; //중복할당 금지.
let h = "hello2";
console.log(h);
