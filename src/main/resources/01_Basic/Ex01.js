...
alert("외부 파일로 선언한 자바스크립트 파일!!!");
/*
 * console.log() 는 주로 디버깅 시에 필요한 정보를 출력할 대 사용함.
 * - 디버깅 : 개발 시 문법적으로 보면 전혀 문제가 없는데도 어떠한 경우에는 정상적으로 동작을 하고, 어떠한 경우에는 동작이제대로 안되는 경우가 발생함.
 * 프로그램에서는 이런 것을 논리적인 오류인 버그라고 부름.
 * 이러한 버그를 찾아서 없애는 작업을 하는 것을 디버깅이라도 말함.
 *
 * console.log()기능은 크롬과 사파리 브라우저에서만 사용이 가능함.
 *
 * html 밖에서도 사용은 가능 하지만 권장 사항은 아님.
 *
 * 자바스크립트에서 데이터를 출력시키는 방법 - 3가지
 * 1. console.log()함수를 이용
 * 2. document.write()함수를 이용하는 방법
 * 	==> HTML의 body 태그에 괄호 안에 있는 내용을 출력해 주는 함수.
 * 		즉, 웹 브라우저의 화면에 내용을 출력해 주는 함수.
 *
 *
*/
document.write("<h2>반갑습니다1. 자바스크립트에 오신걸 환영합니다.</h2>");
document.write("<h2>반갑습니다2. 자바스크립트에 오신걸 환영합니다.</h2>");
document.write("<h2>반갑습니다3. 자바스크립트에 오신걸 환영합니다.</h2>");